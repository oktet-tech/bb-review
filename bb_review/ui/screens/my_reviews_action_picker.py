"""Action picker modal for My Reviews items."""
...
class MyReviewsActionPickerScreen(ModalScreen[tuple[str, list[int]] | None]):
    """Modal for picking an action on My Reviews items.

    Dismisses with (action_key, rr_ids) or None if cancelled.
    Actions: next, ignore, triage, analyze.
    """
    ...
    def compose(self) -> ComposeResult:
        with Container(id="dialog"):
            yield Label("My Reviews Action", id="title")
    ...
    def _select_action(self, action_id: str) -> None:
        if action_id in ("next", "ignore", "triage", "analyze"):
